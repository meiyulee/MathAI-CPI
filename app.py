...
@st.cache_data
def load_excel_data():
    # 支援 cpi_data.xlsx 或 FRED 原始下載檔名
...
# 自動過濾掉不屬於模型月份的分頁（保留名稱包含數字的工作表，如 2026/05）
...
try:
    ...
    if "傳統" in engine_type:
        df_model = df_raw.iloc[:, :15].copy()  # 左側 A-O 欄 (傳統計量版)
    else:
        max_cols = df_raw.shape[1]
        start_idx = min(21, max_cols - 1)
        df_model = df_raw.iloc[:, start_idx:start_idx+11].copy()  # 右側 V-AF 欄 (AI進化版)
        
    # 動態重新命名欄位名稱，避免大小寫不一致閃退
    ...
    # 確保實際 CPI 數據轉為數字，並剔除下方的空白行
    df_model[cpi_col] = pd.to_numeric(df_model[cpi_col], errors='coerce')
    df_clean = df_model.dropna(subset=[cpi_col, date_col])
...
fig.add_trace(go.Scatter(
    x=df_clean[date_col].astype(str), y=df_clean[cpi_col],
...
if beta0 != 0.0 or beta1 != 0.0:
    try:
        ...
        fig.add_trace(go.Scatter(
            x=df_clean[date_col].astype(str), y=y_line,
